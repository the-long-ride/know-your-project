...
class KnowledgeTools:
    def __init__(
        self, *, revisions: ReleaseQueryService, authorization: AuthorizationService
    ) -> None:
    ...
    async def get_feature(
        self, name: str, project: str, release: str | None = None
    ) -> list[ToolResult]:
    ...
    async def get_screen_spec(
        self, screen: str, project: str, release: str | None = None
    ) -> list[ToolResult]:
        return await self.search_project_knowledge(f"screen {screen}", project, release)

    async def get_release_changes(self, release: str, project: str) -> dict[str, object]:
        project_id = self._authorize(project)
