...
def test_public_reference_does_not_depend_on_private_source_id() -> None:
    first = project_result(KnowledgeResult(
        id="fact-x",
        summary="semantic behavior",
        provenance=[SafeProvenance(source_kind="git", source_id="/private/one.cs")],
    ))
    second = project_result(KnowledgeResult(
        id="fact-x",
        summary="semantic behavior",
        provenance=[SafeProvenance(source_kind="git", source_id="/private/two.cs")],
    ))
    assert first["provenance"][0]["reference_id"] == second["provenance"][0]["reference_id"]
